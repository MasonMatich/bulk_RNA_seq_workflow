...
---
# Sample 1. `r sample_params$sample_title` Analysis {.title}
...
pca <- visualizePCsByCondition(
  rlog = rld,
  metadata = colData,
  title = sprintf("Before Regressing out%s", str_to_title(sample_params$batch_cond)))
...
if(as.logical(trimws(sample_params$batch_correct))){
  pca.batch <- visualizePCsByCondition(
      ...
      metadata = colData,
      title = sprintf("After Regressing out%s", str_to_title(sample_params$batch_cond)))
...
# Save PCA to PDF
pdf(sprintf("../figures/%s_PCA.pdf", trimws(sample_params$sample_name)))
...
# Save DESeq Output
saveRDS(dds, file = sprintf("../results/%s_dds.rds", trimws(sample_params$sample_name)))
saveRDS(rld, file = sprintf("../results/%s_rld.rds", trimws(sample_params$sample_name)))

# Save Batch Corrected DESeq Output
if(as.logical(trimws(sample_params$batch_correct))){
  saveRDS(batch.correct.dds, file = sprintf("../results/%s_batch_correct_dds.rds", trimws(sample_params$sample_name)))
  saveRDS(batch.correct.rld, file = sprintf("../results/%s_batch_correct_rld.rds", trimws(sample_params$sample_name)))
}
